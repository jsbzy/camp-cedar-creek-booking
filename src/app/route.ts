...
import { HOMEPAGE_CSS, sitesGridHtml, credentialsHtml } from "@/lib/homepage-extras";
...
export async function GET() {
  try {
    ...
    if (page?.html) {
      const [sites, credentials] = await Promise.all([sitesGridHtml(), credentialsHtml()]);
      return new NextResponse(adaptHomepage(page.html, { extras: { css: HOMEPAGE_CSS, sites, credentials } }), {
        status: 200,
